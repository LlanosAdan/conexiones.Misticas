...
const Links = navBar
  .map(
    (item) => `
            <a href="${item.url}" 
               class="px-3 py-2 text-base lg:text-xl font-semibold tracking-wider font-principal text-white hover:text-[#bf930d]">
               ${item.name}
            </a>
          `
  )
...
headerContainer.innerHTML = `
        <nav class="mx-auto flex max-w-7xl items-center justify-between p-6 lg:px-8">     
          <div class="lg:flex">
            <a href="./" class="text-xl lg:text-5xl font-titulo tracking-widest font-bold text-white">Conexión Mística</a>
          </div>
